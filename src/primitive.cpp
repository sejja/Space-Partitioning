...
**********************************************/
void Primitive::LoadShader(const char* vtxShaderPath, const char* fragShaderPath)
{

    Shader VtxShader(vtxShaderPath, Shader::EType::Vertex);
    Shader FragShader(fragShaderPath, Shader::EType::Fragment);

    if(mShader) delete mShader;
    mShader = new ShaderProgram(VtxShader, FragShader);
